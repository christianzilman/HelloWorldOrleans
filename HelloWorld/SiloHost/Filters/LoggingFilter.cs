        ...
        public async Task Invoke(IIncomingGrainCallContext context)
        {
            try
            {
                if (ShouldLod(context.InterfaceMethod.Name))
                {
                    ...
                    _logger.LogInformation($"LOGGINGFILTER {context.Grain.GetType()}.{context.InterfaceMethod.Name}: arguments: {arguments} request");
                }
                ...
                if (ShouldLod(context.InterfaceMethod.Name))
                {
                    ...
                    _logger.LogInformation($"LOGGINGFILTER {context.Grain.GetType()}.{context.InterfaceMethod.Name}: result: {result} request");
                }
            }
            catch(Exception ex)
            {
                var arguments = JsonConvert.SerializeObject(context.Arguments, _jsonSerializerSettings);
                var result = JsonConvert.SerializeObject(context.Result, _jsonSerializerSettings);
                _logger.LogError($"LOGGINGFILTER {context.Grain.GetType()}.{context.InterfaceMethod.Name}: threw an exception: {nameof(ex)} request", ex);
